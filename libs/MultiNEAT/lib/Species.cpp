...
Genome Species::GetIndividual(Parameters& a_Parameters, RNG& a_RNG) const
{
    ...
    else
    {
        ...
        for(unsigned int i=0; i<t_Evaluated.size(); i++)
            t_probs.push_back( t_Evaluated[i].GetScaledFitness() );
...
Genome Species::GetLeader() const
{
    ...
    for(unsigned int i=0; i<m_Individuals.size(); i++)
    {
        double t_f = m_Individuals[i].GetScaledFitness();
...
void Species::AdjustFitness(Parameters& a_Parameters)
{
    ...
    for(unsigned int i=0; i<m_Individuals.size(); i++)
    {
        double t_fitness = m_Individuals[i].GetScaledFitness();
        ...
        if (m_Individuals[i].GetFitness() > m_BestFitness)
        {
            m_BestFitness = m_Individuals[i].GetFitness();
            m_GensNoImprovement = 0;
        }
...
{
    return ((ls->GetScaledFitness()) > (rs->GetScaledFitness()));
}
