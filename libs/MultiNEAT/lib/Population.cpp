...
void Population::Epoch()
{
    ...
    for(unsigned int i=0; i<m_Species.size(); i++)
    {
        for(unsigned int j=0; j<m_Species[i].m_Individuals.size(); j++)
        {
            if (m_Species[i].m_Individuals[j].GetScaledFitness() > t_bestf)
            {
                t_bestf = m_Species[i].m_Individuals[j].GetScaledFitness();
...
Genome Population::RemoveWorstIndividual()
{
    ...
    for(unsigned int i=0; i<m_Species.size(); i++)
    {
        for(unsigned int j=0; j<m_Species[i].m_Individuals.size(); j++)
        {
            double t_adjusted_fitness = m_Species[i].m_Individuals[j].GetScaledFitness() / static_cast<double>(m_Species[i].m_Individuals.size());
