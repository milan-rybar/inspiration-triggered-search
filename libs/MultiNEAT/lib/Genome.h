...
public:
    ...
    void SetFitness(double a_f) { m_Fitness = a_f; scaledFitness = a_f; }
    void SetAdjFitness(double a_af) { m_AdjustedFitness = a_af; }

    double GetScaledFitness() const { return scaledFitness; }
    void SetScaledFitness(double f) { scaledFitness = f; }
    void SetFitness(double f, double sF) { m_Fitness = f; scaledFitness = sF; }

    int GetDescriptionIndex() const { return descriptionIndex; }
    ...
    {
        return (a_lhs.scaledFitness > a_rhs.scaledFitness);
    }
